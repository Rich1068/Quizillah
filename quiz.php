...
<html>
    ...
    <?php if ($_SESSION['Subject'] === 'Contemporary_World'): ?>
        <body class="bodiCw">
    <?php elseif ($_SESSION['Subject'] === 'Ccs05'):  ?>
        <body class="bodiCcs05">
    <?php elseif ($_SESSION['Subject'] === 'Ite'):  ?>
        <body class="bodiIte">
    <?php elseif ($_SESSION['Subject'] === 'Ccs06'):  ?>
        <body class="bodiCcs06">
    <?php elseif ($_SESSION['Subject'] === 'Ethics'):  ?>
        <body class="bodiEthics">        
    <?php endif; ?>
        ...
        <div align="center">
            <h1 align="center" class="quizh1"><?php echo $question->getNumber(); ?></h1> 
                <form method="POST" action="quiz.php" class="ques">
                    ...
                    <?php endforeach; ?>
                        <?php if ($_SESSION["current_question_number"] < 15){ 
                            echo "<input type='submit' value='Next Question' class='next'>";
                        } else {
                            echo "<input type='submit' onclick='clicked(event)' value='Finish' class='next'>";
                        }?>
                </form>
        </div>
    </body>
